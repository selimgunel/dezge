...
var _ dezge.UserService = (*UserService)(nil)
...
func (s *UserService) FindUserByID(ctx context.Context, id int) (*dezge.User, error) {
...
func (s *UserService) FindUsers(ctx context.Context, filter dezge.UserFilter) ([]*dezge.User, int, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, 0, err
	}
...
func (s *UserService) CreateUser(ctx context.Context, user *dezge.User) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func (s *UserService) UpdateUser(ctx context.Context, id int, upd dezge.UserUpdate) (*dezge.User, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
...
// Returns ENOTFOUND if user does not exist.
func findUserByID(ctx context.Context, tx *Tx, id int) (*dezge.User, error) {
	a, _, err := findUsers(ctx, tx, dezge.UserFilter{ID: &id})
	if err != nil {
		return nil, err
	} else if len(a) == 0 {
		return nil, &dezge.Error{Code: dezge.ENOTFOUND, Message: "User not found."}
	}
	return a[0], nil
}
...
// Returns ENOTFOUND if user does not exist.
func findUserByEmail(ctx context.Context, tx *Tx, email string) (*dezge.User, error) {
	a, _, err := findUsers(ctx, tx, dezge.UserFilter{Email: &email})
	if err != nil {
		return nil, err
	} else if len(a) == 0 {
		return nil, &dezge.Error{Code: dezge.ENOTFOUND, Message: "User not found."}
...
func findUsers(ctx context.Context, tx *Tx, filter dezge.UserFilter) (_ []*dezge.User, n int, err error) {
	// Build WHERE clause.
...
func createUser(ctx context.Context, tx *Tx, user *dezge.User) error {
...
func updateUser(ctx context.Context, tx *Tx, id int, upd dezge.UserUpdate) (*dezge.User, error) {
	// Fetch current object state.
	user, err := findUserByID(ctx, tx, id)
	if err != nil {
		return user, err
	} else if user.ID != dezge.UserIDFromContext(ctx) {
		return nil, dezge.Errorf(dezge.EUNAUTHORIZED, "You are not allowed to update this user.")
	}
...
func deleteUser(ctx context.Context, tx *Tx, id int) error {
	...
	if user, err := findUserByID(ctx, tx, id); err != nil {
		...
		return dezge.Errorf(dezge.EUNAUTHORIZED, "You are not allowed to delete this user.")
	}
