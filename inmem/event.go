...
var _ dezge.EventService = (*EventService)(nil)
...
func (s *EventService) Subscribe(ctx context.Context) (dezge.Subscription, error) {
	// Fetch current user's ID.
	userID := dezge.UserIDFromContext(ctx)
	if userID == 0 {
		return nil, dezge.Errorf(dezge.EUNAUTHORIZED, "Must be logged in to subscribe to events.")
	}
	// Create new subscription for the user.
	sub := &Subscription{
		service: s,
		userID:  userID,
		c:       make(chan dezge.Event, EventBufferSize),
	}
...
// Unsubscribe disconnects sub from the service.
func (s *EventService) LenSubs() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return len(s.m)
}
...
// Ensure type implements interface.
var _ dezge.Subscription = (*Subscription)(nil)
...
func (s *Subscription) C() <-chan dezge.Event {
	return s.c
}
